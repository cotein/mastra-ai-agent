...
const realEstatePropertyFormatterTool = createTool({
  ...
  outputSchema: z.object({
    ...
    mascotas: z.string().describe("Pol\xEDtica de mascotas")
  }),
  execute: async ({ keywordsZonaProp }, _context) => {
    ...
    # REGLAS DE ORO
    1. Si no hay menci\xF3n de mascotas, no hagas menci\xF3n de mascotas.
    ...
    Responde \xDANICAMENTE con un objeto JSON v\xE1lido con las siguientes claves:
    - requisitos: Texto con los requisitos de garant\xEDa, ingresos, etc.
    - mascotas: Texto con la pol\xEDtica de mascotas o cadena vac\xEDa si no se menciona.
    `;
    try {
...
const dynamicInstructions = (datos2, op) => {
  ...
  if (opType === "ALQUILAR") {
    ...
    ***Script Obligatorio***: "${momentoDia}, nico de fausti propiedades por ac\xE1. dale, te ayudo con esa info, \xBFme podr\xEDas decir tu nombre y apellido para agendarte?"
    ...
    - ACCI\xD3N 1 (PRIORIDAD M\xC1XIMA): Informa al cliente los requisitos de la propiedady la pol\xEDtica de mascotas bas\xE1ndote estrictamente en los datos_propiedad.
